...
- Programmatic switching required `string.StartsWith("MaxIterationsReached", …)` — the same anti-pattern Foundry replaced for `NoProvidersAvailableException` with typed exceptions.
...
Adding a new framework case in a future Foundry release is a **soft break** for consumers using exhaustive switch *expressions*:
