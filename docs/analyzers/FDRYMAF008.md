...
A class decorated with `[FoundryAgent]` does not appear in any handoff chain, group chat, or sequential pipeline.
...
Add the agent to an appropriate topology, or remove the `[FoundryAgent]` attribute if the agent is no longer needed.
...
```csharp
#pragma warning disable FDRYMAF008
[FoundryAgent(Instructions = "Standalone utility agent.")]
public class UtilityAgent { }
#pragma warning restore FDRYMAF008
```

## See Also

- [AI Integrations — Multi-Agent Orchestration](../ai-integrations.md#multi-agent-orchestration)
- [FDRYMAF002](FDRYMAF002.md) — group chat has fewer than two members
