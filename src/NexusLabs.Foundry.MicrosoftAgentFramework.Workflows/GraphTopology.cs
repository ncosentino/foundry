...
internal sealed record GraphTopology(
    ...
    Dictionary<Type, IReadOnlyList<Type>> InboundEdges,
    Dictionary<Type, IReadOnlyList<Type>> OutboundEdges,
    ...
    Type? ReducerType)
{
    ...
    /// <summary>
    /// Whether this graph requires the Foundry-native executor instead of the
    ...
    /// advanced feature forces the Foundry-native executor.
    /// </summary>
    public bool RequiresFoundryExecutor =>
        HasWaitAnyNodes ||
        HasConditions ||
        HasOptionalEdges ||
        HasReducer ||
