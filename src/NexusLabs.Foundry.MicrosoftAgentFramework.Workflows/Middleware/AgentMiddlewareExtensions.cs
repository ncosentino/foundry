...
namespace NexusLabs.Foundry.MicrosoftAgentFramework.Workflows.Middleware;

/// <summary>
/// Extension methods that add Foundry middleware to the <see cref="AgentFrameworkBuilder"/>
/// and to MAF's <see cref="AIAgentBuilder"/> directly.
/// </summary>
public static class AgentMiddlewareExtensions
{
    // ─── AgentFrameworkBuilder extensions ────────────────────────────────────
    ...
    /// </remarks>
    public static AgentFrameworkBuilder UsingToolResultMiddleware(
    ...
    /// </para>
    /// <para>
    /// For a custom pipeline use the <see cref="UsingResilience(AgentFrameworkBuilder, Action{ResiliencePipelineBuilder{AgentResponse}})"/>
    /// overload.
    /// </para>
    /// </remarks>
    public static AgentFrameworkBuilder UsingResilience(
        this AgentFrameworkBuilder syringe)
    {
        ArgumentNullException.ThrowIfNull(syringe);
    ...
    // ─── AIAgentBuilder extensions (any-agent, not just Foundry-managed) ──────

    /// <summary>
    /// Adds <see cref="ToolResultFunctionMiddleware"/> to an <see cref="AIAgentBuilder"/>
    /// directly, for agents not managed by Foundry.
    /// </summary>
