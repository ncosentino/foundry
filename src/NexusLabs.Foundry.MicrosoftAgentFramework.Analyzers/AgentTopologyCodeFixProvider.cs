...
{
    private const string Title = "Add [FoundryAgent]";

    public override ImmutableArray<string> FixableDiagnosticIds =>
        ImmutableArray.Create(
            MafDiagnosticIds.HandoffsToTargetNotFoundryAgent,
            MafDiagnosticIds.HandoffsToSourceNotFoundryAgent);
    ...
    public override async Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        ...
        {
            if (diagnostic.Id == MafDiagnosticIds.HandoffsToSourceNotFoundryAgent)
                RegisterSourceFix(context, root, diagnostic);
            else if (diagnostic.Id == MafDiagnosticIds.HandoffsToTargetNotFoundryAgent)
    ...
    private static void RegisterSourceFix(CodeFixContext context, SyntaxNode root, Diagnostic diagnostic)
    {
        ...
        context.RegisterCodeFix(
            CodeAction.Create(
                ...
                equivalenceKey: Title + MafDiagnosticIds.HandoffsToSourceNotFoundryAgent),
            diagnostic);
    }
    ...
    private static async Task RegisterTargetFixAsync(CodeFixContext context, SyntaxNode root, Diagnostic diagnostic)
    {
        ...
        context.RegisterCodeFix(
            CodeAction.Create(
                ...
                equivalenceKey: Title + MafDiagnosticIds.HandoffsToTargetNotFoundryAgent),
            diagnostic);
    }
        ...
        CancellationToken ct)
    {
        ...
        var updated = PrependFoundryAgent(classDeclaration);
    ...
    private static ClassDeclarationSyntax PrependFoundryAgent(ClassDeclarationSyntax classDeclaration)
    {
        var attribute = SyntaxFactory.Attribute(SyntaxFactory.IdentifierName("FoundryAgent"));
