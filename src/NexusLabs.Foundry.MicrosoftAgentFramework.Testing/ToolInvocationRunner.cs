...
/// for Foundry's Agent Framework integration: build a service provider, register the tool, look
...
/// registered by the <c>[ModuleInitializer]</c> the Foundry Agent Framework generator emits in
...
public sealed class ToolInvocationRunner
{
    ...
    /// Provider that has the tool type and its dependencies registered. The Foundry accessors
    ...
    /// <summary>
    /// Creates a runner over a fresh service provider with the Foundry accessors and
    ...
    /// <summary>
    /// Creates a runner over a fresh service provider with only the Foundry accessors registered.
    ...
    /// </summary>
    /// <remarks>
    /// In practice this rarely throws because the Foundry Agent Framework assembly itself emits a
    /// (possibly empty) provider via <c>[ModuleInitializer]</c>, which registers as soon as
    /// <c>NexusLabs.Foundry.MicrosoftAgentFramework.dll</c> loads. The check is still useful as a sanity
    ...
    public void AssertGeneratedProviderAvailable()
    {
        if (!IsGeneratedProviderAvailable)
        {
            throw new InvalidOperationException(
                ...
                "The Foundry Agent Framework source generator did not run for the assembly under test. " +
                "Add a project reference to NexusLabs.Foundry.MicrosoftAgentFramework.Generators with " +
    ...
    public AIFunction GetFunction(Type toolType, string methodName)
    {
        ...
        if (!AgentFrameworkGeneratedBootstrap.TryGetAIFunctionProvider(out var provider))
        {
            throw new InvalidOperationException(
                ...
                "no source-generated IAIFunctionProvider is registered. " +
                "The Foundry Agent Framework source generator did not run for the assembly " +
                $"containing '{toolType.Name}'. Either add a project reference to " +
                "NexusLabs.Foundry.MicrosoftAgentFramework.Generators (Analyzer, no output assembly), " +
        ...
        if (!provider.TryGetFunctions(toolType, resolutionServices, out var functions))
        {
            throw new InvalidOperationException(
                ...
                "[FoundryAgent] FunctionTypes argument) so the generator picks it up.");
        }
