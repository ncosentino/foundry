...
internal sealed class AgentFactory : IAgentFactory
{
    ...
    {
        var attr = agentType.GetCustomAttribute<FoundryAgentAttribute>()
            ?? throw new InvalidOperationException(
                $"'{agentType.Name}' is not decorated with [FoundryAgent]. " +
                $"Apply [FoundryAgent] to the class or use ResolveTools(configure) to set options manually.");
    ...
    private AIAgent CreateAgentFromType(Type agentType, Action<AgentFactoryOptions>? additionalConfigure = null)
    {
        var attr = agentType.GetCustomAttribute<FoundryAgentAttribute>()
            ?? throw new InvalidOperationException(
                $"'{agentType.Name}' is not decorated with [FoundryAgent]. " +
                $"Apply [FoundryAgent] to the class or use CreateAgent(configure) to set options manually.");
