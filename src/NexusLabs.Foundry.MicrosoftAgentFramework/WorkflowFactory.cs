...
internal sealed class WorkflowFactory : IWorkflowFactory
{
    ...
    public Workflow CreateGraphWorkflow(string graphName)
    {
        ...
        // WaitAny is supported via RunGraphAsync which uses Foundry's own executor.
        ...
        foreach (var (sourceType, routingMode) in effectiveRoutingModes)
        {
            if (routingMode == GraphRoutingMode.LlmChoice)
            {
                throw new NotSupportedException(
                    ...
                    $"Foundry's own graph executor with an IChatClient.");
            }
        }
    ...
    /// The <c>IsRequired</c> semantic is a Foundry-native-executor-only feature handled by
    /// <c>RunGraphAsync</c>, which uses Foundry's own graph executor.
    /// </para>
    /// </remarks>
