...
/// <c>"ExampleProduct.Agents"</c>) can set <see cref="MeterName"/> to match, avoiding a
/// dashboard migration when adopting Foundry's <see cref="IAgentMetrics"/>.
/// </para>
/// <para>
...
public sealed class AgentFrameworkMetricsOptions
{
    ...
    /// counters and histograms. Defaults to <c>"NexusLabs.Foundry.MicrosoftAgentFramework"</c>.
    /// </summary>
    public string MeterName { get; set; } = "NexusLabs.Foundry.MicrosoftAgentFramework";
    ...
    /// the <c>gen_ai.client.token.usage</c> histogram on which Foundry emits
    ...
    /// to aggregate Foundry's <c>cache_read</c> / <c>reasoning</c> measurements into the
    ...
    /// Foundry-shape agent metrics (<c>agent.run.*</c>, <c>agent.tool.*</c>, etc.) which
    /// have no upstream cohabitation requirement, while this property scopes the OTel
    /// gen_ai semantic-convention histogram that Foundry shares with MEAI.
    /// </para>
    /// </remarks>
    ...
    /// also active, both Foundry and the upstream middleware create spans for the same
    /// chat completion call. Set this to
    /// <see cref="ChatCompletionActivityMode.EnrichParent"/> to avoid duplicate spans —
    /// Foundry will add its tags (sequence number, char counts, agent name) to the
