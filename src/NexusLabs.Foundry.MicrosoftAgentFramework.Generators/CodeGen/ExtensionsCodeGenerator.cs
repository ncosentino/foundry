...
internal static class ExtensionsCodeGenerator
{
        ...
        string safeAssemblyName)
    {
        ...
        sb.AppendLine("// Foundry AgentFramework IWorkflowFactory Extension Methods");
        sb.AppendLine("#nullable enable");
        sb.AppendLine();
        sb.AppendLine("using Microsoft.Agents.AI.Workflows;");
        sb.AppendLine("using NexusLabs.Foundry.MicrosoftAgentFramework;");
        sb.AppendLine();
        ...
        foreach (var kvp in handoffByInitialAgent.OrderBy(k => k.Key.InitialAgentClassName))
        {
            ...
            sb.AppendLine($"    /// Handoff targets declared via <see cref=\"global::NexusLabs.Foundry.MicrosoftAgentFramework.AgentHandoffsToAttribute\"/>:");
            ...
            if (handoffConditions.Count > 0)
            {
                ...
                sb.AppendLine($"        global::System.Collections.Generic.IReadOnlyList<global::NexusLabs.Foundry.MicrosoftAgentFramework.IWorkflowTerminationCondition> conditions =");
                sb.AppendLine($"            new global::System.Collections.Generic.List<global::NexusLabs.Foundry.MicrosoftAgentFramework.IWorkflowTerminationCondition>");
                sb.AppendLine($"            {{");
                ...
                sb.AppendLine($"            }};");
                sb.AppendLine($"        return await global::NexusLabs.Foundry.MicrosoftAgentFramework.Workflows.StreamingRunWorkflowExtensions.RunAsync(workflow, message, conditions, cancellationToken).ConfigureAwait(false);");
                sb.AppendLine($"    }}");
                sb.AppendLine();
            }
        }
        ...
        foreach (var kvp in groupChatByGroupName.OrderBy(k => k.Key))
        {
            ...
            sb.AppendLine($"    /// </summary>");
            sb.AppendLine($"    /// <remarks>");
            sb.AppendLine($"    /// Participants declared via <see cref=\"global::NexusLabs.Foundry.MicrosoftAgentFramework.AgentGroupChatMemberAttribute\"/>:");
            sb.AppendLine($"    /// <list type=\"bullet\">");
        ...
        foreach (var kvp in sequenceByPipelineName.OrderBy(k => k.Key))
        {
            ...
            sb.AppendLine($"    /// </summary>");
            sb.AppendLine($"    /// <remarks>");
            sb.AppendLine($"    /// Agents declared via <see cref=\"global::NexusLabs.Foundry.MicrosoftAgentFramework.AgentSequenceMemberAttribute\"/> (in order):");
        ...
        foreach (var kvp in graphDataByName.OrderBy(k => k.Key))
        {
            ...
            sb.AppendLine($"    /// </summary>");
            sb.AppendLine($"    /// <remarks>");
            sb.AppendLine($"    /// Topology declared via <see cref=\"global::NexusLabs.Foundry.MicrosoftAgentFramework.AgentGraphEdgeAttribute\"/>:");
            sb.AppendLine($"    /// <list type=\"bullet\">");
            ...
            sb.AppendLine($"    /// </list>");
            sb.AppendLine($"    /// </remarks>");
            sb.AppendLine($"    public static global::Microsoft.Agents.AI.Workflows.Workflow {methodName}(this global::NexusLabs.Foundry.MicrosoftAgentFramework.IWorkflowFactory workflowFactory)");
            ...
            sb.AppendLine($"    /// <summary>");
            sb.AppendLine($"    /// Runs the \"{escapedGraphName}\" graph workflow via <see cref=\"global::NexusLabs.Foundry.MicrosoftAgentFramework.IGraphWorkflowRunner\"/>.");
            ...
            sb.AppendLine($"    /// <returns>An <see cref=\"global::NexusLabs.Foundry.MicrosoftAgentFramework.Diagnostics.IDagRunResult\"/> containing per-node diagnostics and timing.</returns>");
            sb.AppendLine($"    public static async global::System.Threading.Tasks.Task<global::NexusLabs.Foundry.MicrosoftAgentFramework.Diagnostics.IDagRunResult> {runMethodName}(");
            sb.AppendLine($"        this global::NexusLabs.Foundry.MicrosoftAgentFramework.IGraphWorkflowRunner runner,");
            sb.AppendLine($"        string input,");
            sb.AppendLine($"        global::NexusLabs.Foundry.MicrosoftAgentFramework.Progress.IProgressReporter? progress = null,");
            sb.AppendLine($"        global::System.Threading.CancellationToken cancellationToken = default)");
    ...
    public static string GenerateAgentFactoryExtensionsSource(
        List<FoundryAgentTypeInfo> agents,
        Dictionary<string, ImmutableArray<string>> progressSinksByAgent,
        string safeAssemblyName)
    {
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated/>");
        sb.AppendLine("// Foundry AgentFramework IAgentFactory Extension Methods");
        sb.AppendLine("#nullable enable");
        sb.AppendLine();
        ...
        sb.AppendLine("/// Each method creates an agent from its <c>[FoundryAgent]</c> declaration,");
        ...
        foreach (var agent in agents.OrderBy(a => a.ClassName))
        {
            ...
            if (progressSinksByAgent.TryGetValue(agent.ClassName, out var sinkFQNs) && sinkFQNs.Length > 0)
            {
                ...
                sb.AppendLine($"        var sinks = new global::NexusLabs.Foundry.MicrosoftAgentFramework.Progress.IProgressSink[]");
                sb.AppendLine("        {");
                for (int i = 0; i < sinkFQNs.Length; i++)
                {
                ...
                sb.AppendLine("        };");
                sb.AppendLine($"        var factory = global::Microsoft.Extensions.DependencyInjection.ServiceProviderServiceExtensions.GetRequiredService<global::NexusLabs.Foundry.MicrosoftAgentFramework.Progress.IProgressReporterFactory>(sp);");
                sb.AppendLine($"        var accessor = global::Microsoft.Extensions.DependencyInjection.ServiceProviderServiceExtensions.GetRequiredService<global::NexusLabs.Foundry.MicrosoftAgentFramework.Progress.IProgressReporterAccessor>(sp);");
                ...
                sb.AppendLine("        return new global::NexusLabs.Foundry.MicrosoftAgentFramework.Progress.CompositeDisposable(");
        ...
        string safeAssemblyName)
    {
        ...
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated/>");
        sb.AppendLine("// Foundry AgentFramework Progress Sink DI Registration");
        sb.AppendLine("#nullable enable");
        sb.AppendLine();
        ...
        else
        {
            ...
            foreach (var sinkFQN in allSinkFQNs)
            {
                ...
                sb.AppendLine($"        services.AddSingleton<global::NexusLabs.Foundry.MicrosoftAgentFramework.Progress.IProgressSink>(sp => sp.GetRequiredService<{sinkFQN}>());");
            }
        }
        ...
        string safeAssemblyName)
    {
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated/>");
        sb.AppendLine("// Foundry AgentFramework Topology Constants");
        sb.AppendLine("#nullable enable");
        sb.AppendLine();
        sb.AppendLine($"namespace {safeAssemblyName}.Generated;");
        sb.AppendLine();
        ...
        string safeAssemblyName)
    {
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated/>");
        sb.AppendLine("// Foundry AgentFramework AgentFrameworkBuilder Extension Methods");
        sb.AppendLine("#nullable enable");
        sb.AppendLine();
        ...
        sb.AppendLine();
        sb.AppendLine("/// <summary>");
        sb.AppendLine("/// Generated strongly-typed extension methods on <see cref=\"AgentFrameworkBuilder\"/>.");
        sb.AppendLine("/// Each method registers a named function group without requiring direct type references.");
        sb.AppendLine("/// </summary>");
        sb.AppendLine("[global::System.CodeDom.Compiler.GeneratedCodeAttribute(\"NexusLabs.Foundry.MicrosoftAgentFramework.Generators\", \"1.0.0\")]");
        sb.AppendLine("public static class GeneratedAgentFrameworkBuilderExtensions");
        sb.AppendLine("{");
        ...
        foreach (var grp in byGroupName)
        {
            ...
            sb.AppendLine($"    public static AgentFrameworkBuilder {methodName}(this AgentFrameworkBuilder syringe)");
