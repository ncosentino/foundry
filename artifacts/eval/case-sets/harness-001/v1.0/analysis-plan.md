...
The capture key is generated by the runtime from the actual provider messages
and the sampling controls that `EvaluationCaptureChatClient` currently hashes.
It is a lookup/tamper-evidence key, not an independently reconstructable
request-provenance claim: an auditor cannot recompute it without the raw
messages. Configuration provenance instead comes from the canonical input
tuple, including controlling-instruction/tool-schema hashes, package versions,
git SHA, and prompt/compiler implementation version. Prompt or serialization
changes therefore require a new tuple hash/run even when a human-readable case
prompt is unchanged.
...
captures. Advisory judges score the versioned case prompt/reference, normalized
intermediate tool-call/result trajectory, and captured final response artifact;
this protocol does not claim replay of raw intermediate provider requests.
Workspace state, retry state, progress, and diagnostics are retained as
observed evidence and are not reconstructed from the chat-response store.
