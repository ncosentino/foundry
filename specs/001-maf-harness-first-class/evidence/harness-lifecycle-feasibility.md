...
|---|---|---|
| T042 | Canonical relative path-equivalence set (`kb/foo.md`, `./kb/foo.md`, `kb//foo.md`, and backslash-separated relative input resolve identically); rooted/drive/UNC and `..`-traversal rejection; write-then-read round trip; missing-file read through the explicit classifier vs. a surfaced non-missing failure; `FileExistsAsync` canonicalization and fail-closed-on-invalid-path | Every row of the restated bridge matrix has at least one passing assertion; unsupported operations still validate their path before throwing the documented exception type; suite runs against `InMemoryWorkspace` only |
