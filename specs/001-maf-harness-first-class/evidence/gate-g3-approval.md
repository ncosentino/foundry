...
| Forged/mismatched response fails closed | Pass | Two complementary tests document the exact mechanism: (1) with `ApprovalResponseBinding` enabled, `Compose_ForgedResponseWithUnboundRequestId_ResponseBindingDropsItAndFicFailsClosed` shows a fabricated `RequestId` is dropped before `FunctionInvokingChatClient` sees it, producing zero invocations. (2) without approval binding, `Compose_NoApprovalCapabilitySelected_ForgedResponseWithGuessedCallIdStillInvokes` documents MAF's CallId-based baseline and why binding must be selected explicitly. |
...
| Only public MAF 1.15 seams used | Pass | Composition uses the public approval builder extensions, `ChatClientAgentOptions` flags, `ToolApprovalAgent`, approval content types, the `ToolApprovalRequestContent.CreateResponse` instance method, and `CreateAlwaysApproveToolResponse` extension. No internal MAF approval chat-client type is referenced by production code. |
