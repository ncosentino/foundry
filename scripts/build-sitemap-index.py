...
Rationale:
    ...
    knows about at build time. With our split-deploy model (the Documentation
    workflow only sees /api/dev/ + main pages, release.yml only sees the slice being
    ...
    pages are indexed because the Documentation workflow runs far more often
    than release.yml.
    ...
    from the sitemap to avoid indexing duplicate API content. The newest
    archives are mirrored to Cloudflare; older archives remain in gh-pages
    history and the public router redirects unavailable versions to stable.

    Both the Documentation workflow and release.yml invoke this. Neither touches the other's
...
GH_PAGES_API = 'https://api.github.com/repos/ncosentino/foundry/contents/api?ref=gh-pages'
...
def main() -> int:
    ...
    # dev and stable are written by different workflows (Documentation vs release.yml);
