...
def main() -> int:
    ...
    search_index = mirror / 'search' / 'search_index.json'
    removed_search_entries = 0
    if search_index.is_file():
        search_data = json.loads(search_index.read_text(encoding='utf-8'))
        original_docs = search_data.get('docs', [])
        retained_docs = []
        for document in original_docs:
            ...
            if (
                len(parts) >= 2
                and parts[0] == 'api'
                and is_version_name(parts[1])
                and parts[1] not in keep_paths
            ):
                removed_search_entries += 1
                continue
            retained_docs.append(document)

        search_data['docs'] = retained_docs
        search_index.write_text(
            json.dumps(search_data, separators=(',', ':')),
            encoding='utf-8',
        )

    print(
        f'Kept {len(keep_paths)} version archive(s); '
        f'removed {removed} older archive dir(s) and '
        f'{removed_search_entries} stale search entries.'
    )
    return 0
